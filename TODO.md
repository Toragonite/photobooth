...
**Tasks:**
- [x] Implement `SessionRepository` interface with SQLAlchemy
- [x] Implement `PrintJobRepository` interface with SQLAlchemy
- [x] Add entity-to-model and model-to-entity mapping functions
- [x] Handle async database operations
...
**Tasks:**
- [x] Create container class with all service registrations
- [x] Create FastAPI `Depends()` functions for each use case
- [x] Configure singleton vs request-scoped services

### 3. Update Infrastructure Services ✅
Make existing services implement the port interfaces.

**Files modified:**
- `backend/app/infrastructure/services/printer_service.py` → implements `PrinterPort`
- `backend/app/infrastructure/services/storage_service.py` → implements `StoragePort`
- `backend/app/infrastructure/services/image_processor.py` → implements `ImageProcessorPort`
- `backend/app/infrastructure/services/system_service.py` → implements `SystemServicePort`

**Tasks:**
- [x] Add port interface inheritance to each service class
- [x] Ensure method signatures match port definitions
- [x] Add any missing methods required by ports
...
4. **Controller Refactoring** - Final step, can be done incrementally (NEXT)
