...
print_reminders() {
    ...
    echo ""
    echo "3. Verify Wi-Fi password is secure:"
    echo "   - Check /etc/hostapd/hostapd.conf"
    echo "   - Ensure wpa_passphrase is set to a strong password"
    echo "   - Restart hostapd if changed: sudo systemctl restart hostapd"
    echo ""
